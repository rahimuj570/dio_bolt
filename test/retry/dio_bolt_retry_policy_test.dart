...
void main() {
  group('DioBoltRetryPolicy - Tests', () {
    ...
    DioException makeTransportError(
      ...
      return DioException(requestOptions: req, type: type);
    }

    test(
      '1. Retries default eligible status codes: 408, 429, 500, 502, 503, 504',
      () async {
    ...
    test(
      '2. Does NOT retry non-eligible status codes: 400, 401, 403, 404, 405, 409, 422',
      () async {
    ...
    test(
      '3. Retries eligible transport errors: timeout, connectionError',
      () async {
        final eligibleTypes = [
        ...
        for (final type in eligibleTypes) {
          ...
          expect(
            should,
            isTrue,
            reason: 'Transport error $type should be retryable',
          );
        }
      },
    );
    ...
    test(
      '4. Does NOT retry non-eligible transport errors: badCertificate, cancel',
      () async {
        ...
        for (final type in ineligibleTypes) {
          ...
          expect(
            should,
            isFalse,
            reason: 'Transport error $type must NOT be retryable',
          );
        }
      },
    );
    ...
    test(
      '5. Method safety: GET, HEAD, OPTIONS, PUT, DELETE retry by default; POST, PATCH rejected',
      () async {
        for (final method in ['GET', 'HEAD', 'OPTIONS', 'PUT', 'DELETE']) {
          ...
          expect(
            should,
            isTrue,
            reason: 'Method $method should be retryable by default',
          );
        }
        ...
        for (final method in ['POST', 'PATCH']) {
          ...
          expect(
            should,
            isFalse,
            reason: 'Method $method must NOT be retryable by default',
          );
        }
      },
    );
    ...
    test(
      '6. Per-request override: copyWithRetry(enabled: true) permits POST retry',
      () async {
    ...
    test(
      '7. Per-request override: copyWithRetry(enabled: false) disables GET retry',
      () async {
    ...
    test('8. Body replayability: Safe bodies allow retry', () async {
      ...
      for (final body in safeBodies) {
        ...
        expect(
          should,
          isTrue,
          reason: 'Body ${body.runtimeType} should be replayable',
        );
      }
    });

    test('8b. Generic arbitrary FormData is rejected from retry', () async {
      final genericFormData = FormData.fromMap({'field': 'value'});
      final err = makeHttpError(503, data: genericFormData);
      final should = await policy.shouldRetry(err, err.requestOptions, 0);
      expect(
        should,
        isFalse,
        reason: 'Generic arbitrary FormData must NOT be retried',
      );
    });

    test(
      '9. Body replayability: Unsafe one-shot streams and finalized files REJECT retry even with override',
      () async {
        ...
        expect(
          ...
          isFalse,
          reason:
              'One-shot stream must NOT be retried even with explicit override',
        );
      },
    );

    test(
      '10. Body replayability: Unknown/custom body types REJECT retry even with override',
      () async {
        final customObject = _UnknownCustomPayload('some data');
        final overrideExtra = Options().copyWithRetry(enabled: true).extra;

        final err = makeHttpError(
          503,
          data: customObject,
          extra: overrideExtra,
        );
        ...
        expect(
          ...
          isFalse,
          reason:
              'Unknown custom object must NOT be retried even with explicit override',
        );
      },
    );
    ...
    test('11. Max retries limit enforcement', () async {
      ...
      expect(
        await policy.shouldRetry(err, err.requestOptions, 0),
        isTrue,
      ); // attempt 0 -> retry 1
      expect(
        await policy.shouldRetry(err, err.requestOptions, 1),
        isTrue,
      ); // attempt 1 -> retry 2
      expect(
        await policy.shouldRetry(err, err.requestOptions, 2),
        isTrue,
      ); // attempt 2 -> retry 3
      expect(
        await policy.shouldRetry(err, err.requestOptions, 3),
        isFalse,
    ...
    test('12. Custom retryEvaluator with (error, options, attempt)', () async {
      ...
      final reqAllowed = RequestOptions(
        path: '/test',
        headers: {'X-Retry-Allowed': 'true'},
      );
      final errAllowed = DioException(
        requestOptions: reqAllowed,
        response: Response(requestOptions: reqAllowed, statusCode: 500),
      );
      expect(await customPolicy.shouldRetry(errAllowed, reqAllowed, 0), isTrue);

      final reqDenied = RequestOptions(
        path: '/test',
        headers: {'X-Retry-Allowed': 'false'},
      );
      final errDenied = DioException(
        requestOptions: reqDenied,
        response: Response(requestOptions: reqDenied, statusCode: 500),
      );
