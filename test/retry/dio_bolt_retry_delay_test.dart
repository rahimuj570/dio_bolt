...
void main() {
  group('DioBoltRetryDelay - Tests', () {
    ...
    test('calculates exponential backoff without jitter', () {
      ...
      expect(calculator.calculateDelay(attempt: 1, config: config), equals(const Duration(milliseconds: 100)));
      // attempt 2: 100 * (2^1) = 200ms
      expect(calculator.calculateDelay(attempt: 2, config: config), equals(const Duration(milliseconds: 200)));
      // attempt 3: 100 * (2^2) = 400ms
      expect(calculator.calculateDelay(attempt: 3, config: config), equals(const Duration(milliseconds: 400)));
      // attempt 4: 100 * (2^3) = 800ms
      expect(calculator.calculateDelay(attempt: 4, config: config), equals(const Duration(milliseconds: 800)));
    });
    ...
    test('clamps delay to maxDelay', () {
      ...
      // attempt 1: 1s
      expect(calculator.calculateDelay(attempt: 1, config: config), equals(const Duration(seconds: 1)));
      // attempt 2: 3s -> clamped to 2s
      expect(calculator.calculateDelay(attempt: 2, config: config), equals(const Duration(seconds: 2)));
      // attempt 3: 9s -> clamped to 2s
      expect(calculator.calculateDelay(attempt: 3, config: config), equals(const Duration(seconds: 2)));
    });
    ...
    test('clamps 429 Retry-After to maxDelay when clampRetryAfterToMaxDelay is true', () {
      const config = DioBoltRetryConfig(
        maxDelay: Duration(seconds: 2),
        respectRetryAfter: true,
