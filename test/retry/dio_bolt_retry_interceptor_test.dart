...
void main() {
  group('DioBoltRetryInterceptor - Integration Tests', () {
    ...
    test(
      '3. Max retries exhausted: returns final normalized 503 response envelope',
      () async {
        int requestCount = 0;
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
        dio.httpClientAdapter = MockAdapter((options) async {
        ...
        expect(
          requestCount,
          equals(3),
        ); // 1 initial + 2 retries (total 3 attempts)
      },
    );
    ...
    test(
      '5. Cancellation during retry delay aborts immediately without sending retry request',
      () async {
        int requestCount = 0;
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
        dio.httpClientAdapter = MockAdapter((options) async {
        ...
        final bolt = DioBolt(
          ...
          retryConfig: DioBoltRetryConfig(
            maxRetries: 3,
            delayCalculator: (attempt, i, m, mult, j, err) =>
  ...
  group('DioBoltRetryInterceptor - Auth & Retry Interaction Flows', () {
    test(
      'Flow A: 503 -> network retry -> 200 (Auth does not interfere)',
      () async {
        int requestCount = 0;
        int refreshCount = 0;
    ...
    test(
      'Flow B: 401 -> auth refresh -> auth retry -> 200 (Network retry does not interfere)',
      () async {
        int requestCount = 0;
        int refreshCount = 0;
    ...
    test(
      'Flow C: 503 -> network retry -> 401 -> auth refresh -> auth retry -> 200',
      () async {
        int requestCount = 0;
        int refreshCount = 0;
        ...
        expect(response.statusCode, equals(200));
        expect(
          response.responseData['token_used'],
          equals('Bearer fresh-token'),
        );
        expect(
          requestCount,
          equals(3),
        ); // 1 initial + 1 network retry + 1 auth retry
        expect(refreshCount, equals(1));
      },
    );

    test(
      'Flow D: 401 -> auth refresh -> auth retry -> 503 -> network retry -> 200',
      () async {
        int requestCount = 0;
        int refreshCount = 0;
        ...
        expect(
          requestCount,
          equals(3),
        ); // 1 initial + 1 auth retry + 1 network retry
        expect(refreshCount, equals(1));
      },
    );
  });
