...
void main() {
  group('DioBoltRefreshManager - Unit Tests', () {
    test(
      'single-flight executes refreshToken callback exactly once for concurrent requests',
      () async {
    ...
    test(
      'persistence failure treats refresh as failed and triggers onRefreshFailed exactly once',
      () async {
    ...
    test(
      'fromDio creates isolated Dio client without recursive interceptors',
      () {
        ...
        expect(
          manager.refreshClient.options.baseUrl,
          equals('https://api.example.com'),
        );
        expect(
          manager.refreshClient.interceptors
              .whereType<DioBoltAuthInterceptor>(),
          isEmpty,
        );
        expect(
          manager.refreshClient.interceptors.contains(customInterceptor),
          isFalse,
        );
