...
void main() {
  ...
  group('DioBoltAuth - Header Precedence & Case-Insensitivity', () {
    test(
      '1. Preserves explicitly supplied Authorization header on initial request',
      () async {
        RequestOptions? captured;
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
        dio.httpClientAdapter = MockAdapter((options) async {
        ...
        expect(
          captured?.headers['Authorization'],
          equals('Bearer custom-explicit-token'),
        );
      },
    );

    test(
      '2. Case-insensitively detects explicit authorization header',
      () async {
        RequestOptions? captured;
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
        dio.httpClientAdapter = MockAdapter((options) async {
        ...
        expect(
          captured?.headers['authorization'],
          equals('Bearer lowercase-token'),
        );
        expect(
          captured?.headers['authorization'],
          isNot(contains('auto-token')),
        );
      },
    );

    test(
      '3. Explicit Authorization participates in 401 refresh and retry updates to new token',
      () async {
        int requestCount = 0;
  ...
  group('DioBoltAuth - 401 Refresh & Retry Lifecycle', () {
    test(
      'Single 401 refreshes token, calls onTokenRefreshed, and retries successfully',
      () async {
        int requestCount = 0;
        int refreshCount = 0;
        ...
        dio.httpClientAdapter = MockAdapter((options) async {
          ...
          return jsonBody({
            'user': 'Alice',
            'token_received': options.headers['Authorization'],
          });
        });
        ...
        expect(response.statusCode, equals(200));
        expect(
          response.responseData['token_received'],
          equals('Bearer new-token-999'),
        );
    ...
    test(
      'Retry-once protection: retried request receiving 401 does NOT trigger second refresh',
      () async {
        int requestCount = 0;
        int refreshCount = 0;
  ...
  group('DioBoltAuth - Single-Flight Concurrency & Exactly-Once Callback', () {
    test(
      '5 concurrent 401 requests trigger exactly 1 refresh call and all 5 succeed',
      () async {
        int requestCount = 0;
        int refreshCount = 0;
        ...
        expect(
          refreshCount,
          equals(1),
        ); // Single-flight: exactly 1 refresh HTTP call
    ...
    test(
      '10 concurrent 401 requests with refresh failure invoke onRefreshFailed EXACTLY ONCE',
      () async {
        int refreshCount = 0;
        ...
        expect(
          failureResponseReceived?.responseData,
          equals({'code': 'SESSION_EXPIRED'}),
        );
    ...
    test(
      'Single-flight state resets cleanly after failure allowing subsequent wave to retry',
      () async {
        int refreshCount = 0;
  ...
  group('DioBoltAuth - Failure Semantics & Strict Persistence', () {
    test(
      'Strict Persistence: when onTokenRefreshed throws, refresh is deemed FAILED',
      () async {
    ...
    test(
      'refreshToken throwing exception is caught safely and does not escape public API',
      () async {
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
        dio.httpClientAdapter = MockAdapter((options) async {
          return jsonBody({'error': 'Unauthorized'}, statusCode: 401);
        ...
        final bolt = DioBolt(
          ...
          auth: DioBoltAuth(
            getAccessToken: () => 'token',
            refreshToken: (_) =>
                throw Exception('Network socket died during refresh'),
          ),
        );
    ...
    test(
      'onRefreshFailed throwing exception does not escape public API',
      () async {
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
        dio.httpClientAdapter = MockAdapter((options) async {
          return jsonBody({'error': 'Unauthorized'}, statusCode: 401);
        ...
        final bolt = DioBolt(
          ...
          auth: DioBoltAuth(
            ...
            refreshToken: (_) => null,
            onRefreshFailed: (_) =>
                throw Exception('Crash inside logout callback'),
          ),
        );
  ...
  group('DioBoltAuth - Cancellation Safety & Independence', () {
    test(
      'Cancellation while waiting for refresh unblocks cancelled request immediately without stopping shared refresh',
      () async {
        final dio = Dio(BaseOptions(baseUrl: 'https://api.example.com'));
  ...
  group('DioBoltAuth - Payload Replayability', () {
    ...
    test('Generic FormData payload is NOT retried on 401', () async {
      int count = 0;
    ...
    test(
      'Stream payload is NOT retried and returns original 401 response',
      () async {
        int count = 0;
  ...
  group('DioBoltAuth - Isolated Refresh Client & Multi-Instance', () {
    test(
      'Refresh client 401 does not trigger recursive refresh loop',
      () async {
        int mainCount = 0;
        int refreshCount = 0;
        ...
        dio.httpClientAdapter = MockAdapter((options) async {
          if (options.path == '/auth/refresh') {
            refreshCount++;
            return jsonBody({
              'error': 'Refresh token also expired',
            }, statusCode: 401);
          }
          mainCount++;
  ...
  group('DioBoltAuth - Retry Options Preservation', () {
    test(
      'Preserves query parameters, custom headers, contentType, and extra on retry',
      () async {
        int count = 0;
        ...
        expect(
          retriedOptions?.headers['X-Custom-Tracking'],
          equals('track-99'),
        );
        expect(
          retriedOptions?.headers['Authorization'],
