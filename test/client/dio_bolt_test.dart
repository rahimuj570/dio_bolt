...
void main() {
  ...
  group('DioBolt Raw API - Success HTTP responses (2xx)', () {
    ...
    test('201 POST returns DioBoltResponse with isSuccess: true', () async {
      final response = await bolt.post('/status/201', data: {'name': 'Bob'});
    ...
    test(
      '204 DELETE returns DioBoltResponse with isSuccess: true and null responseData',
      () async {
    ...
    test(
      'Preserves complete complex nested structure without reduction',
      () async {
    ...
    test('Query parameters reach Dio correctly', () async {
      ...
      expect(
        ...
        equals({'filter': 'active', 'page': 1}),
      );
    });

    test(
      'Header precedence: explicit headers override Options.headers',
      () async {
        final options = Options(
          headers: {'Authorization': 'Bearer old-token', 'X-Keep': 'preserved'},
        );
    ...
    test(
      'Send progress and receive progress callbacks are triggered',
      () async {
        int? sentBytes;
        int? receivedBytes;
  ...
  group('DioBolt Raw API - HTTP error responses (4xx & 5xx) never throw', () {
    ...
    test(
      '400 returns DioBoltResponse with isSuccess: false and statusCode: 400',
      () async {
        ...
        expect(
          ...
          equals({'error': 'Bad Request', 'field': 'email'}),
        );
    ...
    test(
      '401 returns DioBoltResponse with isSuccess: false and statusCode: 401',
      () async {
    ...
    test(
      '403 returns DioBoltResponse with isSuccess: false and statusCode: 403',
      () async {
    ...
    test(
      '404 returns DioBoltResponse with isSuccess: false and statusCode: 404',
      () async {
    ...
    test(
      '409 returns DioBoltResponse with isSuccess: false and statusCode: 409',
      () async {
    ...
    test(
      '429 returns DioBoltResponse with isSuccess: false and statusCode: 429',
      () async {
        ...
        expect(
          response.responseData,
          equals({'error': 'Too Many Requests', 'retryAfter': 60}),
        );
      },
    );

    test(
      '500 returns DioBoltResponse with isSuccess: false and statusCode: 500',
      () async {
    ...
    test(
      '503 returns DioBoltResponse with isSuccess: false and statusCode: 503',
      () async {
  ...
  group('DioBolt Raw API - Transport and network failures never throw', () {
    ...
    test(
      'connection timeout returns statusCode: 0, isSuccess: false, message: Connection timed out.',
      () async {
    ...
    test(
      'send timeout returns statusCode: 0, isSuccess: false, message: Request sending timed out.',
      () async {
    ...
    test(
      'receive timeout returns statusCode: 0, isSuccess: false, message: Response receiving timed out.',
      () async {
    ...
    test(
      'connection error (no internet / DNS) returns statusCode: 0, isSuccess: false, message: Unable to connect to the server.',
      () async {
    ...
    test(
      'cancel returns statusCode: 0, isSuccess: false, message: Request was cancelled.',
      () async {
    ...
    test(
      'bad certificate returns statusCode: 0, isSuccess: false, message: Secure connection could not be established.',
      () async {
        ...
        expect(
          response.message,
          equals('Secure connection could not be established.'),
        );
      },
    );

    test(
      'unknown failure returns statusCode: 0, isSuccess: false with sensible message',
      () async {
        setupTransportError(
          DioExceptionType.unknown,
          customMessage: 'Socket closed unexpectedly',
        );
        final response = await bolt.get('/test');
  ...
  group('DioBoltResponse unit tests', () {
    test('toString produces descriptive string', () {
      ...
      expect(
        response.toString(),
        equals(
          'DioBoltResponse(statusCode: 200, isSuccess: true, message: OK, responseData: {key: value})',
        ),
      );
    });

    test(
      'fromDioException handles badResponse with null statusMessage using default fallback',
      () {
